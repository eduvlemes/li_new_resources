...
(function () {
    ...
    const DEFAULT_CONFIG = {
        ...
        productPage: {
            ...
            // keywords:     busca por palavra-chave definida em parameter (/_search?nocache=' + Math.random() +'&q=...)
            ...
            fixedProductSkus: [],    // alternativa a fixedProductIds: se preenchido, a busca dos fixos e por SKU (1 fetch por SKU)
    ...
    // field: 'product_ids' (padrao) ou 'sku'. A API aceita 1 valor por chamada nos dois casos.
    function fetchProductInfo(value, field) {
        return fetch('/_search?nocache=' + Math.random() +'&an=' + window.LOJA_ID + '&filters=' + (field || 'product_ids') + ':' + encodeURIComponent(value))
    ...
    function fetchCategoryProducts(categoryId) {
        return fetch('/_search?nocache=' + Math.random() +'&an=' + window.LOJA_ID + '&filter=is_active:true;is_searchable:true;is_available:true;category_ids:' + encodeURIComponent(categoryId) + '&sort=units_sold:desc')
            .then(function (r) { return r.ok ? r.json() : null; })
            .then(function (data) {
    ...
    function fetchProductsByIds(ids, field) {
        var promises = ids.map(function (id) { return fetchProductInfo(id, field); });
        return Promise.all(promises).then(function (results) {
            return results.filter(Boolean);
        });
    }

    // Busca produtos por uma única palavra-chave via /_search?nocache=' + Math.random() +'&q=
    function fetchKeywordsProducts(parameter) {
        if (!parameter) return Promise.resolve([]);
        return fetch('/_search?nocache=' + Math.random() +'&an=' + window.LOJA_ID + '&q=' + encodeURIComponent(parameter) + '&filter=is_active:true;is_available:true')
            .then(function (r) { return r.ok ? r.json() : null; })
            .then(function (data) {
                if (!data || !Array.isArray(data.products)) return [];
    ...
    function fetchProductMainCategory(productId) {
        return fetch('/_search?nocache=' + Math.random() +'&an=' + window.LOJA_ID + '&filters=product_ids:' + encodeURIComponent(productId))
            .then(function (r) { return r.ok ? r.json() : null; })
            .then(function (data) {
    ...
    function resolveProducts(sectionCfg, contextData, excludeIds) {
        // Se fixedProductSkus estiver preenchido, busca por SKU; senao, por ID
        var fixedSkus  = (sectionCfg.fixedProductSkus || []).map(String);
        var fixedField = fixedSkus.length > 0 ? 'sku' : 'product_ids';
        var fixedIds   = fixedSkus.length > 0 ? fixedSkus : (sectionCfg.fixedProductIds || []).map(String);
        var sort     = sectionCfg.sort || 'most_sold';
        var max      = sectionCfg.maxProducts || 6;

        return Promise.all([
            fixedIds.length > 0 ? fetchProductsByIds(fixedIds, fixedField) : Promise.resolve([]),
    ...
    function openSkuModal(product, onSkuSelected) {
        ...
        sortedSkus.forEach(function (sku) {
            ...
            var label = sku.varLabel || (sku.variations[0].value.value || sku.variations[0].value.name);
