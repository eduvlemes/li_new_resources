...
(function () {
    ...
    class AutoCoupon {
        ...
        // HTML retornado, evitando reload. Se a resposta não trouxer a tabela
        // (ex.: remover cupom), busca a própria página de carrinho pra pegá-la.
        // Fora da página de carrinho, recarrega.
        finish(html) {
            ...
            const incoming = this.parseTable(html);
            if (incoming) { swap(incoming); return; }

            // Resposta sem tabela: recarrega o carrinho e extrai de lá.
            fetch(window.location.href, {
                credentials: 'same-origin',
                headers: { 'X-Requested-With': 'XMLHttpRequest' }
            })
                .then(r => r.text())
                .then(pageHtml => swap(this.parseTable(pageHtml)))
    ...
    function initAutoCoupon() {
        // 2s após a tela carregada
        const start = () => setTimeout(() => { window.AutoCoupon = new AutoCoupon(); }, 2000);
