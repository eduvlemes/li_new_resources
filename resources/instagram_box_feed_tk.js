...
(function () {
    ...
    const DEFAULT_CONFIG = {
        ...
        // Cache dos dados da API do Instagram na sessionStorage
        // true  → guarda o resultado na sessão do browser (evita chamadas repetidas ao navegar)
        // false → sempre consulta a API
        // A chave de cache inclui um hash do accessToken — troca de token invalida automaticamente.
        sessionCache: true,
    ...
    class InstagramFeed {
        ...
        _loadDataFromInstagram() {
            ...
            // ---- Cache na sessionStorage ----
            const CACHE_KEY = 'ig_feed_cache_' + this._tokenHash(token);
            if (CONFIG.sessionCache) {
                ...
                } catch (_) { /* sessionStorage indisponível ou JSON inválido — ignora */ }
            }
            ...
            .then(([profile, media]) => {
                ...
                if (CONFIG.sessionCache) {
                    try { sessionStorage.setItem(CACHE_KEY, JSON.stringify(data)); } catch (_) { /* cota esgotada — ignora */ }
                }
        ...
        // Gera uma chave curta a partir do token para identificar o cache sem expor o token
        _tokenHash(token) {
            let h = 0;
