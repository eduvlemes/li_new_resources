 ...
 * configuráveis e salva as respostas no localStorage com a chave:
...
(function () {
    ...
    const DEFAULT_CONFIG = {
        ...
        // Prefixo opcional para a chave do localStorage.
        ...
        // MODO INLINE
        // Quando true, exibe os campos diretamente na página (antes do elemento indicado),
        // em vez de abrir um modal popup.
        // inlineContainerSelector define o elemento de referência: os campos são inseridos ANTES dele.
        inlineMode: false,
        inlineContainerSelector: ".produto .acoes-produto:first-child",
    ...
    const CSS_STYLES = `
        <style id="product-customizer-styles">
            ...
            .pc-inline-form .pc-header-texts {
                padding: 0 0 4px;
            }

            .pc-inline-form .pc-form-step {
                display: flex;
                flex-direction: column;
                gap: 16px;
            }
    ...
    class ProductCustomizer {
        ...
        _injectInlineFields() {
            const targets = document.querySelectorAll(CONFIG.inlineContainerSelector);
            if (!targets.length) return;

            const firstTarget = targets[0];
            if (firstTarget.previousElementSibling && firstTarget.previousElementSibling.classList.contains('pc-inline-form')) return;

            const fieldsHTML = CONFIG.fields.map(field => this._buildFieldHTML(field)).join('');

            const titleHTML = CONFIG.showInlineTitle
                ? `<div class="pc-header-texts">
                    <h2 class="pc-modal-title">${this._esc(CONFIG.modalTitle)}</h2>
                    ${CONFIG.modalSubtitle ? `<p class="pc-modal-subtitle">${this._esc(CONFIG.modalSubtitle)}</p>` : ''}
                   </div>`
                : '';

            const html = `
                <div class="pc-inline-form" id="pc-inline-form">
                    ${titleHTML}
                    <div class="pc-form-step">
                        ${fieldsHTML}
                    </div>
                </div>
            `;

            firstTarget.insertAdjacentHTML('beforebegin', html);

            const inlineForm = firstTarget.previousElementSibling;
            this._setupConditionals(inlineForm);
            this._updateConditionals(inlineForm);
        }
        ...
        setupEventListeners() {
            if (CONFIG.inlineMode) {
                // MODO INLINE: intercepta clique, valida e salva campos inline
                document.addEventListener('click', (e) => {
                    ...
                    const inlineForm = document.getElementById('pc-inline-form');
                    ...
                    if (!this._validateFields(inlineForm)) return;

                    const properties = this._collectValues(inlineForm);
                    ...
                    if (productId) {
                        ...
                        try {
                            ...
                            console.warn('[ProductCustomizer] Erro ao salvar no localStorage:', err);
                        }
                    }

                    const savedBtn = this.currentButton;
                    this.currentButton = null;
                    setTimeout(() => {
        ...
        _handleConfirm() {
            ...
            if (productId) {
                ...
                try {
                    ...
                    console.warn('[ProductCustomizer] Erro ao salvar no localStorage:', e);
                }
            }
        ...
        _setupConditionals(container = this.overlay) {
            ...
            triggerIds.forEach(triggerId => {
                container.querySelectorAll(`[name="${triggerId}"]`).forEach(input => {
                    input.addEventListener('change', () => this._updateConditionals(container));
                });
            });
        }

        _updateConditionals(container = this.overlay) {
            CONFIG.fields.forEach(field => {
                if (!field.showIf) return;

                const fieldContainer = container.querySelector(`.pc-field[data-field-id="${field.id}"]`);
                if (!fieldContainer) return;

                const { field: triggerFieldId, value: triggerValue, values: triggerValues } = field.showIf;
                const currentValue = this._getFieldCurrentValue(triggerFieldId, container);
                ...
                fieldContainer.classList.toggle('pc-field--hidden', !show);
            });
        }

        _getFieldCurrentValue(fieldId, container = this.overlay) {
            ...
            if (fieldConfig.type === 'radio') {
                ...
                return [...container.querySelectorAll(`input[name="${fieldId}"]:checked`)].map(c => c.value);
            } else if (fieldConfig.type === 'checkbox') {
                const el = container.querySelector(`#pc-field-${fieldId}`);
        ...
        _collectValues(container = this.overlay) {
            const values = {};

            CONFIG.fields.forEach(field => {
                // Ignora campos atualmente ocultos por condição showIf
                const fieldContainer = container.querySelector(`.pc-field[data-field-id="${field.id}"]`);
                if (fieldContainer && fieldContainer.classList.contains('pc-field--hidden')) return;
                ...
                if (field.type === 'radio') {
                    ...
                    const el = container.querySelector(`#pc-field-${field.id}`);
        ...
        _validateFields(container = this.overlay) {
            ...
            CONFIG.fields.forEach(field => {
                ...
                let hasValue = true;
                const errorEl = container.querySelector(`#pc-error-${field.id}`);
                const inputEl = container.querySelector(`#pc-field-${field.id}`);

                if (field.type === 'radio') {
                    hasValue = !!container.querySelector(`input[name="${field.id}"]:checked`);
                } else if (field.type === 'checkbox' && field.options && field.options.length > 0) {
                    hasValue = !!container.querySelector(`input[name="${field.id}"]:checked`);
                } else if (field.type === 'checkbox') {
        ...
        _clearErrors(container = this.overlay) {
            container.querySelectorAll('.pc-error').forEach(el => el.classList.remove('pc-error'));
            container.querySelectorAll('.pc-error-msg').forEach(el => {
                el.textContent = '';
        ...
        destroy() {
            ...
            document.querySelectorAll('.pc-inline-form').forEach(el => el.remove());
